...
QUnit.test('Testing calculateArea function with several sets of inputs', function (assert) {
    assert.equal(calculateArea(4,2), 8, 'Tested with two relatively small positive numbers');
    assert.equal(calculateArea(-3,0), 0, 'Tested with two negative or 0 integers return 0');
    assert.equal(calculateArea(0,101), 0, 'Tested with one or two "too large" integers return 0');
    assert.equal(calculateArea(NaN , NaN), null, 'Tested with missing or invalid arguments return null ');
    assert.equal(calculateArea(parseInt(9.4), parseInt(5.4)), 45, 'Tested with 10.4 and 2.3 to get 45');
    assert.throws(function () {calculateArea("Christine","Christine"); }, /The given argument is not a number/, 'Passing in a string correctly raises an Error');
});
