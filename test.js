...
QUnit.test('Testing calculateArea function with several sets of inputs', function (assert) {
    assert.throws(function (){ calculateArea(4,2);}, 8, 'Tested with two relatively small positive numbers');
    assert.throws(function (){ calculateArea(-3,-3);}, 0, 'Tested with two negative or 0 integers return 0');
    assert.throws(function (){ calculateArea(2, );},0, 'Tested with missing or invalid arguments return null ');
     assert.throws(function () {calculateArea("you","me"); }, /given argument is not a number/, 'Passing in a string correctly raises an Error');
});
